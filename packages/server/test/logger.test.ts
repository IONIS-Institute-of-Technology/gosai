...
/** Background writes must finish before the test deletes their directory. */
function track(logger: Logger): Logger {
  loggers.push(logger);
  return logger;
}
...
describe('Logger', () => {
  test('persists to backlog and notifies subscribers', () => {
    const logger = track(new Logger({ logsDir: newDir() }));
    const got: string[] = [];
  ...
  test('respects minLevel', () => {
    const logger = track(new Logger({ logsDir: newDir(), minLevel: 'warn' }));
    const got: string[] = [];
    logger.subscribe((e) => got.push(e.message));
  ...
  test('writes entries in the background, in order', async () => {
    const dir = newDir();
    const logger = track(new Logger({ logsDir: dir }));
  ...
  test('rotates the file and deletes the oldest rotated files', async () => {
    const dir = newDir();
    const logger = track(new Logger({ logsDir: dir, maxBytes: 200, maxFiles: 2 }));
